...
function card(code) {
  ...
  const chunks = db.prepare("SELECT * FROM chunks WHERE tbl = ? AND (code = ? OR codes LIKE ?) ORDER BY CASE WHEN fid IN (SELECT fid FROM docs WHERE kind='kokuji') THEN 0 ELSE 1 END").all(tbl, code, `%"${code}"%`);
  const qas = db.prepare('SELECT q.* FROM refs r JOIN qa q ON q.id = r.owner WHERE r.code = ? ORDER BY q.doc, q.no').all(code);
  ...
  for (const c of chunks) {
    hr(`${c.code || '通則'} ${c.title}  ― ${docLabel(c.fid)} p${c.p_start}${c.p_end !== c.p_start ? '-' + c.p_end : ''}`);
...
if (!words.length) { console.error('使い方: node bin/ask.js <区分番号 | 検索語 ...>'); process.exit(1); }
const code = words.length === 1 ? parseCode(words[0]) : null;
if (code) card(code); else search(words);
