...
// 調剤の通知は「区分００ 調剤基本料」のように「区分」を前置する
const CODE_HEAD_RE = new RegExp(`^(?:区分)?(${CODE_LIST})(?:[\\s　]+(.*)|([^\\s　０-９0-9－‐―\\-の、にをはが及か].*))?$`);
...
// 調剤通知の「＜調剤技術料＞」のような山括弧見出し（節相当）
const BRACKET_HEAD_RE = /^[＜<]([^＜＞<>]{1,20})[＞>][\s　]*$/;
...
function isSectionHeading(m, x, headX) {
  ...
  return title.length <= 30 && !/[、。]|に掲げ|に規定|の各区分|通則/.test(title); // 「第12部 歯冠修復及び欠損補綴」の「及び」は許容
}
...
function buildStructure(textJson, opts = {}) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const bm = t.match(BRACKET_HEAD_RE);
    if (bm && !TSUSOKU_RE.test(t) && l.x <= headX + 40) {
      pathState['節'] = bm[1]; pathState['款'] = null;
      sections.push({ level: '節', title: bm[1], p: l.p, line: i });
      closeChunk(i - 1);
      continue;
    }
